 ...
 */
module.exports.createProject = async function(inputInfo, defaultModuleName = config.defaultModuleName) {
  const {
    name,
    version,
    description,
  ...
  const mainVersion = await getVersionOfSyncMe();
  ...
  const packageInfo = {
    ...
    license,
    devDependencies: {
      [config.mainPackageName]: mainVersion,
...
async function getVersionOfSyncMe() {
  ...
  return rv || versions[0] || config.mainVersion;
}

 ...
 * 创建项目
 */
module.exports.createProject = async function(options) {
  ...
  if (options.notInstall !== undefined) {
    finalLogs.unshift('\nFinished!\n');
...
module.exports.startProject = () => {
  const gfPath = path.resolve(process.cwd(), `node_modules/${config.mainPackageName}/gulpfile.js`);
  execProcess(`gulp --gulpfile "${gfPath}" --watch`);
}
...
module.exports.buildProject = () => {
  const gfPath = path.resolve(process.cwd(), `node_modules/${config.mainPackageName}/gulpfile.js`);
  execProcess(`gulp --gulpfile "${gfPath}"`);
}
